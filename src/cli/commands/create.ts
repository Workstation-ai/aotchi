...
import { registerAgent, getOrCreateWallet, getBalance, type Network } from "../../infra/erc8004.js";
...
export const createCommand = new Command("create")
  ...
  .action(async (name: string, opts: { skill: string; description?: string; deploy?: boolean; network: Network }) => {
    console.log(banner());

    const skill = SKILLS[opts.skill];
    if (!skill) {
      console.error(
        NEON.red(`  ✗ Unknown skill "${opts.skill}"`)
      );
      console.error(
        NEON.dim(`  Available: ${Object.keys(SKILLS).join(", ")}`)
      );
      process.exit(1);
    }
    ...
    if (opts.deploy) {
      ...
      console.log(aotchiCard(aotchi.state));
      console.log(
        NEON.dim(`  Next: ${NEON.cyan(`aotchi feed ${name} 10`)} to bring it to life`)
      );
      console.log(
        NEON.dim(`  Or:   ${NEON.cyan(`aotchi create ${name} --skill ${opts.skill} --deploy`)} for on-chain identity`)
      );
    }
    console.log();
  });
