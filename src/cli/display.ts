...
import { AotchiState, SKILLS } from "../core/types.js";
...
const STATUS_STYLES: Record<string, { icon: string; color: (s: string) => string }> = {
...
export function aotchiCard(state: AotchiState): string {
...
export function aotchiListCard(state: AotchiState): string {
  const style = STATUS_STYLES[state.status] || STATUS_STYLES.egg;
  const skill = SKILLS[state.skill];
...
export function walletCard(balance: number, address?: string): string {
...
export function eggAnimation(name: string): string {
  return [
    ``,
...
export function starvationWarning(name: string): string {
  return boxen(
    [
